            ...
            <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex gap-2">
                    {['All', 'Paid', 'Pending', 'Fulfilled'].map(f => (
                        <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${filter === f ? 'bg-brand-dark text-white' : 'bg-gray-200 text-gray-500 hover:bg-gray-100'}`}>{f}</button>
                    ))}
                </div>
            </div>
...
const ProductsView = () => {
    ...
    // Use union type for field to avoid TS errors
    const handleVariantChange = (id: string, field: keyof AdminVariant | string, value: any) => {
